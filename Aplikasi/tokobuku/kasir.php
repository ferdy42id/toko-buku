					...
					if(isset($_GET['action'])){
						if($_GET['action'] == 'input'){
							...
							if(!isset($_GET['id'])){
								...
								<input type=\"text\" name=\"namaKasir\" value=\"".$Kasir->getNama()."\" required>
								</div>
								<div class=\"form-group\">
								<label>Alamat</label>
								<input type=\"text\" name=\"alamat\" value=\"".$Kasir->getAlamat()."\" required>
								</div>
								<div class=\"form-group\">
								<label>Telpon</label>
								<input type=\"number\" name=\"telp\" value=\"".$Kasir->getTelp()."\" required>
								</div>
								<div class=\"form-group\">
								<label>Username</label>
								<input type=\"text\" name=\"username\" value=\"".$Kasir->getUsername()."\" required>
								</div>
								<div class=\"form-group\">
								<label>Password</label>
								<input type=\"password\" name=\"password\" value=\"".$Kasir->getPassword()."\" required>
								</div>
								<div class=\"form-group\">
								<label>Status</label>
								<select name=\"status\">
								<option value=\"1\" ".($Kasir->getStatus() == "1" ? "selected" : "").">Aktif</option>
								<option value=\"0\" ".($Kasir->getStatus() == "0" ? "selected" : "").">Tidak Aktif</option>
								</select>
								</div>
								<div class=\"form-group\">
								<label>Level</label>
								<select name=\"level\">
								<option value=\"admin\" ".($Kasir->getLevel() == "admin" ? "selected" : "").">Admin</option>
								<option value=\"staff\" ".($Kasir->getLevel() == "staff" ? "selected" : "").">staff</option>
								</select>
								</div>
								<div class=\"form-group\">
								<input type=\"submit\" name=\"submit\">
